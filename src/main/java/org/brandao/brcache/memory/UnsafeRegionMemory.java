...
public class UnsafeRegionMemory implements Serializable{
	...
	public long read(UnsafeRegionMemory buf, long off, long len){
		long trans = Math.min(this.length - this.off, len);
	...
	public void write(UnsafeRegionMemory buf, long off, long len){
		long trans = Math.min(this.length - this.off, len);
